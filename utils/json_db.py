...
def _blob_put_json(path: str, data: Any) -> None:
    ...
    access = _blob_access()
    attempts = [access]
    if "private" not in attempts:
        attempts.append("private")
    if "public" not in attempts:
        attempts.append("public")

    last_exc = None
    last_body = ""

    for access_value in attempts:
        try:
            response = _put_with_access(access_value)
            ...
            if response_obj is not None:
                last_body = (response_obj.text or "")[:500]
            continue
    else:
        import sys
        print(f"Blob write error for {pathname}: {last_exc} | body={last_body}", file=sys.stderr)
        extra = f" | response={last_body}" if last_body else ""
        raise RuntimeError(f"Failed to write JSON to Vercel Blob path '{pathname}': {last_exc}{extra}") from last_exc
